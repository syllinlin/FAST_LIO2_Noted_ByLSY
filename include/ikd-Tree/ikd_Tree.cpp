...
/// @brief 构建kd-tree
/// @tparam PointType 
/// @param point_cloud 用来构建kd-tree的点云
template <typename PointType>
void KD_TREE<PointType>::Build(PointVector point_cloud)
{
    // 如果根节点不为空，但是需要重新构建kd-tree，那么需要删除之前的根结点
    if (Root_Node != nullptr)
    {
        delete_tree_nodes(&Root_Node);
    }
...
/// @brief 根据点云构建kd-tree中的各节点
/// @tparam PointType 
/// @param root 根结点
/// @param l = 0
/// @param r 点云总数量——用于构建当前子树的点云数量
/// @param Storage 点云集合
template <typename PointType>
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage)
{
    ...
    *root = new KD_TREE_NODE;
    ...
    // 遍历点云集合，找到最小和最大的x y z ，
    // 注意最后min_value中的三个值极大可能来自是不同点云，max_value同理
    for (i = l; i <= r; i++)
    {
    ...
    // 根据划分轴进行数据左右划分，由此递归构建
    (*root)->division_axis = div_axis; // 记录当前结点的划分轴，用于后续查询和搜索
    // 根据划分轴，进行相应轴上的坐标排序
    switch (div_axis)
    {
        case 0:
...
void KD_TREE<PointType>::Update(KD_TREE_NODE *root)
{
    ...
    {
        // 当前结点树的大小（总结点数量） = 左子树大小 + 右子树大小 + 1 （1是指当前结点）
        // 通过递归构建的树，所以回溯的时候，是知道子树大小的
        root->TreeSize = left_son_ptr->TreeSize + right_son_ptr->TreeSize + 1;
        root->invalid_point_num = left_son_ptr->invalid_point_num + right_son_ptr->invalid_point_num + (root->point_deleted ? 1 : 0); // 需要额外判断根节点是否需要被删除
        root->down_del_num = left_son_ptr->down_del_num + right_son_ptr->down_del_num + (root->point_downsample_deleted ? 1 : 0); // 需要额外判断根节点是否在降采样的过程中被删除
        // 判断根节点是否被降采样删除
        ...
        // 判断该树是否被删除
        // 根据当前根节点自身属性以及左右子树情况，统计当前树的范围（通常称之为BOX）
    ...
    // 计算以该节点为中心，在BOX中的最大圈的范围radius_sq
