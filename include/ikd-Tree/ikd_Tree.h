...
class KD_TREE
{
    ...
    struct KD_TREE_NODE
    {
        PointType point; // PointType = pcl::PointXYZINormal
        int division_axis; // 分割轴
        int TreeSize = 1; // 树的大小初始化为1
        int invalid_point_num = 0; // 有效的节点树量初始化为0
        int down_del_num = 0; // TODO：下采样需要删除的节点树量
        bool point_deleted = false; // 该节点是否需要删除
        bool tree_deleted = false; // 以该节点为根节点的子树是否要被删除

        bool point_downsample_deleted = false; // 点节点是否在降采样的时候被删除
        bool tree_downsample_deleted = false; // 以该节点的树，是否在降采样的时候被删除
        bool need_push_down_to_left = false; // TODO：该节点是否需要左旋到左子树 
        bool need_push_down_to_right = false; // TODO：该节点是否需要右旋到右子树
        ...
        float radius_sq;
        KD_TREE_NODE *left_son_ptr = nullptr; // 左孩子初始化为空
        KD_TREE_NODE *right_son_ptr = nullptr; // 右孩子初始化为空
        KD_TREE_NODE *father_ptr = nullptr; // 父节点初始化为空
        // For paper data record
        float alpha_del; // 用于评价当前树是否需要进行结点删除
        float alpha_bal; // 用于评价当前树是否低于最小的平衡度要求
    };
    ...
    private:
    // Multi-thread Tree Rebuild —— 多线程树重建
    bool termination_flag = false;
    bool rebuild_flag = false; // 是都进行重建
    ...
    void InitTreeNode(KD_TREE_NODE *root);  // 初始化结点——相关属性初始化
    void Test_Lock_States(KD_TREE_NODE *root);
    void BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage); // 构建K-d树
    void Rebuild(KD_TREE_NODE **root); // 重建以root为根结点的树，旨在维持一个很好的平衡因子
